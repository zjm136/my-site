...
export async function onRequestPost({request,env}){
  ...
  try{
    const r=await fetch('https://gitee.com/api/v5/repos/'+env.GITEE_OWNER+'/'+name+'?access_token='+env.GITEE_TOKEN,{method:'DELETE'});
    if(!r.ok&&r.status!==404) return json({ok:false,msg:'删除失败 '+r.status},500);
    const key='gitee:repos';
    let list=JSON.parse(await env.USER_DB.get(key)||'[]');
    list=list.filter(x=>x.name!==name);
    if(list.length&&list.every(x=>!x.is_default)) list[0].is_default=true;
    await env.USER_DB.put(key,JSON.stringify(list));
    return json({ok:true,msg:'已删除仓库 '+name});
